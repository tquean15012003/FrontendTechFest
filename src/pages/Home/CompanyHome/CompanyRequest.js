...
export default function CompanyRequest() {
    ...
    const { user, requestReceivedList } = useSelector(state => state.UserReducer);
    ...
    const renderRequestHeader = (request) => {
        return (
            <h2 className="text-2xl font-bold title-font mb-2 text-white">{_.capitalize(request.requestType)}</h2>
        )
    }
    ...
    const renderRequestInfo = (request) => {
        return Object.keys(JSON.parse(request.data)).map((item, index) => {
            ...
            if (item !== "issuer") {
                return (
                    <p className="text-white leading-relaxed text-base" key={index}><span className="font-bold text-2xl">{_.capitalize(item)}:</span> <span className="text-xl">{JSON.parse(request.data)[item]}</span></p>
                )
            }
